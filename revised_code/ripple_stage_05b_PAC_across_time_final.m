...
%%
% [ripple_stage_05b_PAC_across_time] find ripples in hippocampal channels,
% extract and realign data based on ripple events.
% estimate phase-amplitude coupling across time around ripples
...
settings.PAC_phase       = [1:10]; % frequencies for phase data
settings.PAC_power       = 30:5:150; % frequencies for power data
...
settings.nu_time_points         = 500; % time of sliding window in ms
...
if settings.peak_frequency == 1
    ...
    freq_theta = [theta_freq(max_theta)-1:theta_freq(max_theta)+1];
    freq_gamma = [settings.PAC_power(max_gamma)-5:settings.PAC_power(max_gamma)+5];
...
elseif settings.peak_frequency == 2
    ...
    if settings.bs_correct == 1 % based on prestim baselin
        ...
        trlinfo_ripplelocked    = cell2mat(tfr_ripples.trialinfo);
        
        sel_trl                 = intersect([trlinfo_BL.EventNumber],[trlinfo_ripplelocked.EventNumber]);
        sel_chan                = intersect(data_BL.label,tfr_ripples.label);
        ...
        tfr_BL.trialinfo = tfr_ripples.trialinfo;
        
        %--- power for baseline period
        
        pow_BL   = abs(tfr_BL.powspctrm) .^2;
    ...
    %% pick cortical and hippocampal channels
    these_hipp = intersect(tfr_ripples.label,intersect(cellstr(setdiff([data.label],settings.scalp_channels)),char(channels_hipp_ripples{isubject,:})));
    these_cort = setdiff(tfr_ripples.label,these_hipp);
    ...
    for icond = 1:1
        ...
        fs              = 1/(tfr_ripples_theta.time(2)-tfr_ripples_theta.time(1));
        chunk_size      = round(settings.nu_time_points/((1/fs)*1000)); % Size of each chunk
        overlap_size    = round(chunk_size * settings.prc_overlap);
        num_chunks      = floor((size(tfr_ripples_gamma.powspctrm, 4) - overlap_size) / (chunk_size - overlap_size));
        TOI_ripple      = linspace(tfr_ripples_theta.time(1), tfr_ripples_theta.time(end),num_chunks);
        ...
        if settings.PAC_or_MI == 1
            ...
            for itime = 1:num_chunks
                    ...
                    for ipow = 1:numel(FOI_power)
                        ...
                        for ichan = 1:size(pow_ripples_PAC,2)
                            for iichan = 1:size(plv_ripples_PAC,2)
                                ...
                                ampl = squeeze(pow_ripples_PAC(:,ichan,FOI_power(ipow),start_idx:end_idx));
                                phas = squeeze(plv_ripples_PAC(:,iichan,FOI_phase(ifreq),start_idx:end_idx));
                ...
                PAC_all_time(itime,:,:) = PAC_all;
                if any(settings.PAC_nusurro)
                    PAC_perm_all_time(itime,:,:)    = PAC_perm_all;
                end
                    ...
                    for ipow = 1:numel(FOI_power)
                        ...
                        for ichan = 1:size(pow_ripples_PAC,2)
                            for iichan = 1:size(plv_ripples_PAC,2)
                                ...
                                ampl = squeeze(pow_ripples_PAC(:,ichan,FOI_power(ipow),start_idx:end_idx));
                                phas = squeeze(exp(1i*plv_ripples_PAC(:,iichan,FOI_phase(ifreq),start_idx:end_idx)));
        ...
        if icond == 1
            PAC{isubject}.correct   = PAC_all_time;
        elseif icond == 2
            PAC{isubject}.incorrect   = PAC_all_time;
        end
        if any(settings.PAC_nusurro)
            PAC{isubject}.permuted  = PAC_perm_all_time;
        end
        PAC{isubject}.time = TOI_ripple;
    end
% clearvars -except PAC isubject settings subjects SubjectIDs
end
...
data_freq_pow = {};
data_freq_pow{1,1} = phase_to_plot;
data_freq_pow{2,1} = pow_to_plot;

figure;
subplot(1,2,1)
h   = rm_raincloud(data_freq_pow(1), [settings.colour_scheme_1(7,:)],0,'ks',[],settings.colour_scheme_1);
h.p{1, 1}.FaceColor = settings.colour_scheme_1(3,:);
yticklabels({' '})
xlabel('frequency (hz)')
title('theta')
set(gca,'TickDir','out')
...
hold on
subplot(1,2,2)
h   = rm_raincloud(data_freq_pow(2), [settings.colour_scheme_1(7,:)],0,'ks',[],settings.colour_scheme_1);
h.p{1, 1}.FaceColor = settings.colour_scheme_1(21,:);
yticklabels({' '})
xlabel('frequency (hz)')
title('gamma')
hold on
...
cfg.numrandomization    = 'all';%1000;%'all';
...
axis xy
set(gca,'FontSize',12)
xlabel('time around ripple')
ylabel('frequency phase')
set(gca,'TickDir','out')
...
set(gca, 'YTickLabel', arrayfun(@num2str, freqaxis, 'UniformOutput', false));
...
d = squeeze(mean(correct_all-baseline_all,2));

m = squeeze(nanmean(d));
s = squeeze(nanstd(d)./sqrt(size(d,1)));
...
[a,b,c,d] = ttest(squeeze(mean(correct_all(:,:,:),2)),squeeze(mean(baseline_all(:,:,:),2)), 'tail', 'right');

plot(timeaxis,d.tstat,'LineWidth',  3)
...
ylabel('t-values')
set(gca,'TickDir','out')
title('t-values, time-resolved PAC')
