...
for isubject = 1:size(settings.subjects,1)
    ...
    powspctrm(isubject,:) = perf{isubject}.powspctrm/ max(perf{isubject}.powspctrm);
end

% restrict to gamma search range, e.g. 40–140 Hz
gamma_idx = freqaxis >= 55 & freqaxis <= 140;
f_gamma   = freqaxis(gamma_idx);

FOI = nearest(freqaxis,55):nearest(freqaxis,140);

m = mean(powspctrm(:,gamma_idx));
s = std(powspctrm(:,gamma_idx))/sqrt(12);

boundedline(f_gamma,m, s,'k','alpha')
...
load perf_delta_theta_ripple_aligned

perf = perf_delta_theta_ripple_aligned;
% phase

theta_foi = nearest(perf{1}.freq,.5):nearest(perf{1}.freq,2);
time_idx = -1:1/250:1;


correct_all_trials_phase   = [];
incorrect_all_trials_phase = [];
all_trials_phase = cell(size(settings.subjects,1),1); 


for isubject = 1:size(settings.subjects,1)

    trlinfo = cell2mat(perf{isubject}.trlinfo);

    sel1 = ismember([trlinfo.Memory], 'SourceCorrect');
    sel2 = ismember([trlinfo.Memory], {'SourceIncorrect','SourceDunno','dunno'});
    sel3 = sel1 | sel2;

    % ----------------------------------------------------------
    % Extract complex delta/theta Fourier values: 
    %   trials × channels × freq × time
    % ----------------------------------------------------------
    F = perf{isubject}.phase;  

    % ----------------------------------------------------------
    % Average across freq (complex average)
    %   → trials × channels × time
    % ----------------------------------------------------------
    F_favg = squeeze(mean(F(:,:,theta_foi,:), 3));

    phase_tc = angle(F_favg); % trials × channels × time 
    all_trials_phase{isubject} = phase_tc; % keep subject-specific sizes end
end

time_idx = -1:1/250:1;     % same as before
[~, t0] = min(abs(time_idx - 0));   % ripple peak index

all_mean_phase_chan = [];  % 1 × (all channels across subjects)

for isubject = 1:numel(all_trials_phase)

    phase_tc = all_trials_phase{isubject};  % trials × channels × time

    % extract phase at ripple peak → trials × channels
    phi = squeeze(phase_tc(:,:,t0));

    % circular mean across trials for each channel → 1 × nChan
    mean_phase = angle(mean(exp(1i*phi), 1));

    % concatenate across subjects
    all_mean_phase_chan = [all_mean_phase_chan, mean_phase];

end

% Rayleigh test for non-uniformity
[p, z] = circ_rtest(all_mean_phase_chan);
fprintf('Rayleigh: z = %.3f, p = %.3g\n', z, p);

nSurr = 1000;
z_surr = zeros(nSurr,1);
surrogate_phases_all = [];
for s = 1:nSurr
    surrogate_phases = [];
    for isubject = 1:numel(all_trials_phase)
        phase_tc = all_trials_phase{isubject};
        [nTr,nCh,nT] = size(phase_tc);
        
        shift = randi([1 nT]); % random circular phase shift
        phase_surr = phase_tc(:,:,mod((1:nT)+shift-1,nT)+1);
        
        phi_surr = squeeze(phase_surr(:,:,t0));
        mean_surr = angle(mean(exp(1i*phi_surr),1));
        surrogate_phases = [surrogate_phases, mean_surr];
    end
    [~, z_surr(s)] = circ_rtest(surrogate_phases);
    surrogate_phases_all = [surrogate_phases_all;surrogate_phases];
end
...
polarhistogram(surrogate_phases_all, 18, 'FaceColor', [0.3 0.3 0.3], 'EdgeColor',[0.3 0.3 0.3], 'FaceAlpha',.2,'EdgeAlpha',.2,'Normalization', 'probability');
hold on
polarhistogram(all_mean_phase_chan, 18, 'FaceColor', settings.colour_scheme_1(23,:) , 'EdgeColor','none','Normalization', 'probability');
