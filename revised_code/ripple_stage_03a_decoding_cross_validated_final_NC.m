...
parfor isubject = 1:numel(subjects)
    ...
    for icond = 1:2
        ...
        K = 5; % (5 or 10 is more typical than 3, but 3 is ok if trial counts are low)
        cv = cvpartition(clabelTraining,'KFold',K);
        ...
        acc_maps = [];
         for k = 1:K
             ...
             Xte = dataToClassifyTraining(teIdx,:,:);
             yte = clabelTraining(teIdx);
             ...
             [accuracy, ~]               = mv_classify_timextime(cfg, Xte, yte, dataToClassifyTest, clabelTest);
        ...
        if icond == 1
            perf{isubject}.correct.accuracy   = squeeze(mean(acc_maps));
        elseif icond == 2
            perf{isubject}.incorrect.accuracy = squeeze(mean(acc_maps));
        end
    ...
    for icond = 1:2
        ...
        if icond == 1
             if size(acc_maps,1) == 1
                perf{isubject}.colour.correct.accuracy         = squeeze(acc_maps);
             else
                perf{isubject}.colour.correct.accuracy         = squeeze(mean(acc_maps));
             end
        ...
        elseif icond == 2
            ...
            if size(acc_maps,1) == 1
                perf{isubject}.colour.incorrect.accuracy       = squeeze(acc_maps);
            else
                perf{isubject}.colour.incorrect.accuracy       = squeeze(mean(acc_maps));
            end
    ...
    for icond = 1:2
        ...
        if icond == 1
            ...
            if size(acc_maps,1) == 1
                perf{isubject}.scene.correct.accuracy         = squeeze(acc_maps);
            else
                perf{isubject}.scene.correct.accuracy         = squeeze(mean(acc_maps));
            end
        ...
        elseif icond == 2
            ...
            if size(acc_maps,1) == 1
                perf{isubject}.scene.incorrect.accuracy         = squeeze(acc_maps);
            else
                perf{isubject}.scene.incorrect.accuracy       = squeeze(mean(acc_maps));
            end
